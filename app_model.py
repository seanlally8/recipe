...
def update_tables(title, instructions_title, instructions_body, ingredients_list, url):
    """
    This function takes all the information scraped from the url and enters it into
    the database.
    """

    # Insert recipe title into titles table (title_id is autogenerated) 
    stmt = insert(titles).values(title=title.strip(), url=url)
    ...
    for ingredient in ingredients_list:
        ...
        if ingredient.strip().startswith("Salt"):
            break
        stmt = insert(ingredients).values(ingredient=ingredient.strip(), title_id=title_id)
        connection.execute(stmt)
    connection.commit()

    # Insert instructions into instructions table
    for (entry, title) in zip(instructions_body, instructions_title):
        stmt = insert(instructions).values(instruction=entry.strip(),
                                            instruction_title=title.strip(),
                                            title_id=title_id)
        connection.execute(stmt)
    connection.commit()
