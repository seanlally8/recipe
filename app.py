...
def index():
    ...
    if request.method == "POST":
        ...
        # the recipe to the database
            ...
            # Gather the image from the post request
            images = request.files.getlist("image")

            # Gather the recipe title from the post request
            recipe_title = request.form.get("title")

            # Declare lists here to avoid UnboundLocalError 
            ingredients_list = []
            ...
            for image in images:
                ...
                # Extract extension for each image
                ...
                for recipe_str in recipe_strings:
                    ...
                    if "\u00BC" in recipe_str.lower() or "\u00BD" in recipe_str.lower() or "sea salt" in recipe_str.lower():
            ...
            if not len(ingredients_list) or not len(list_of_instructions):
                return report_error("Highly uncertain about that image. Resubmit material, glareless and straight.")
            ...
            update_tables(recipe_title, list_of_instructions, ingredients_list, None)
